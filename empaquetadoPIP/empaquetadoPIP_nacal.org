...
# cd $(HOME)/$(name)/; python3 -m twine upload --repository testpypi dist/*

# ficheros:
# cd $(DIRECTORIO_NACAL); make todo
# cd $(DIRECTORIO_NACAL); make init

# cp  README.md $(HOME)/$(name)/
...
** TODO Cositas por mirar                                          :noexport:

- [X] mirar como convertir md a org

- [X] mirar como exportar bloque a md

- [ ] eliminar target =ficheros=
  + [ ] mirar como hacer tangle desde fuera del fichero org
  + [ ] mirar como hacer exportar a pdf desde fuera del fichero org
...
Convertir =README.org= a Github Flavored Markdown 
#+name: CrearMarkdown
...
** COMMENT Otras formas de generar README from org

- https://jeffkreeftmeijer.com/org-readme/

#+BEGIN_SRC sh  :results none
emacs --batch README.org --funcall org-md-export-to-markdown
#+END_SRC

Convertir =README.org= a markdown (standard)
#+BEGIN_SRC sh :results none
pandoc -f markdown -t org -o README.org README.md
#+END_SRC

** Hacer tangle desde bash

#+BEGIN_SRC sh :results silent
emacs --batch --eval "(require 'org)" --eval '(org-babel-tangle-file "empaquetadoPIP_nacal.org")'
#+END_SRC
